...
#%%%% Read data (set column diag_malign where B is set to be 0, M is set to be 1)
dta_cancer = pd.read_csv('data_cancer_benigno_maligno.csv')
dta_cancer["diag_malign"] = dta_cancer["diagnosis"].replace(["B","M"],[0,1])


#%%%% Split the data
dta_cancer['ML_group']   = np.random.randint(100,size = dta_cancer.shape[0])
dta_cancer               = dta_cancer.sort_values(by='ML_group').reset_index()
inx_train                = dta_cancer.ML_group<80                     
inx_valid                = (dta_cancer.ML_group>=80)&(dta_cancer.ML_group<90)
inx_test                 = (dta_cancer.ML_group>=90)


#%%%% Set x matrix
scaler = preprocessing.StandardScaler()
feature_names = dta_cancer.columns.tolist()[3:-2]
X = scaler.fit_transform(dta_cancer[feature_names].values)


#%%%% TVT-splitting
Y_train = dta_cancer.diag_malign[inx_train].to_list()
Y_valid = dta_cancer.diag_malign[inx_valid].to_list()
Y_test  = dta_cancer.diag_malign[inx_test].to_list()

X_train = X[np.where(inx_train)[0],:]
X_valid = X[np.where(inx_valid)[0],:]
X_test  = X[np.where(inx_test) [0],:]

right_to_total_dict1 = {}
...
dta_cancer['diag_hat_reg'] = np.concatenate(
        [
...
dta_cancer.loc[dta_cancer['diag_hat_reg']>1,'diag_hat_reg'] = 1
dta_cancer.loc[dta_cancer['diag_hat_reg']<0,'diag_hat_reg'] = 0

# confusion matrix for linear regression
conf_matrix1 = np.zeros([2,2])
for i in range(2):
    for j in range(2):
        conf_matrix1[i,j] = np.sum((dta_cancer[inx_test].diag_malign == i)*(dta_cancer[inx_test].diag_hat_reg == j ))
...
right_to_total_dict1["Linear regression"] = ratio_reg


#%%%% Naive Bayes Classification
clf2 = GaussianNB().fit(X_train, Y_train)
dta_cancer['diag_hat_NB'] = np.concatenate(
        [
                clf1.predict(X_train),
                clf1.predict(X_valid),
                clf1.predict(X_test)
...
dta_cancer.loc[dta_cancer['diag_hat_NB']>1,'diag_hat_NB'] = 1
dta_cancer.loc[dta_cancer['diag_hat_NB']<0,'diag_hat_NB'] = 0

# confusion matrix for  Naive Bayes
conf_matrix2 = np.zeros([2,2])
for i in range(2):
    for j in range(2):
        conf_matrix2[i,j] = np.sum((dta_cancer[inx_test].diag_malign == i)*(dta_cancer[inx_test].diag_hat_NB == j))
...
right_to_total_dict1["Naive Bayes"] = ratio_nb
...
for criterion in criterion_chosen:
    ...
    for depth in range(2,max_depth):
        ...
        dta_results_tree["diag_malign"] = dta_cancer.diag_malign.copy()
        ...
        for i in range(2):
            for j in range(2):
                conf_matrix3[i,j] = np.sum((dta_results_tree[dta_results_tree.inx_valid].diag_malign == i) * (dta_results_tree[dta_results_tree.inx_valid][depth-2] == j))
...
for i in range(2):
    for j in range(2):
        conf_matrix_tree[i,j] = np.sum((dta_results_tree[dta_results_tree.inx_test].diag_malign == i) * (dta_results_tree[dta_results_tree.inx_test][best_depth-2] == j))
...
right_to_total_dict1["Trees"] = ratio_tree
...
for k in range(1,max_k_nn):
    ...
    dta_results_knn["diag_malign"] = dta_cancer.diag_malign.copy()
    ...
    for i in range(2):
        for j in range(2):
            conf_matrix4[i,j] = np.sum((dta_results_knn[dta_results_knn.inx_valid].diag_malign == i) * (dta_results_knn[dta_results_knn.inx_valid][k-1] == j))
...
for i in range(2):
    for j in range(2):
        conf_matrix_knn[i,j] = np.sum((dta_results_knn[dta_results_knn.inx_test].diag_malign == i) * (dta_results_knn[dta_results_knn.inx_test][best_k-1] == j))
...
right_to_total_dict1["KNN"] = ratio_knn
...
dta_cancer['diag_hat_lasso']             = np.concatenate(
        [
...
dta_cancer.loc[dta_cancer['diag_hat_lasso']>1,'diag_hat_lasso'] = 1
dta_cancer.loc[dta_cancer['diag_hat_lasso']<0,'diag_hat_lasso'] = 0

# confusion matrix for lasso
conf_matrix5 = np.zeros([2,2])
for i in range(2):
    for j in range(2):
        conf_matrix5[i,j] = np.sum((dta_cancer[inx_test].diag_malign == i)*(dta_cancer[inx_test].diag_hat_lasso == j))    
...
right_to_total_dict1["Lasso"] = right_to_total


#%%%% Compare which classifier is the best here
print(right_to_total_dict1)

# predictive model that gives the best oos prediction
best_mod = [model for model, value in right_to_total_dict1.items() if value == max(right_to_total_dict1.values())]
        
# print the maximum right_to_total
print("")
max_rrt = max(right_to_total_dict1.values())
